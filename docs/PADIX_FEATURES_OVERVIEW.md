...
**Порядок pin'ов в чате (по дате игры):** после каждого нового pin вызывается `reorderPinsByDate(chatId)`, которая для всех открытых будущих pin'ов в чате делает `unpin → pin` в порядке от далёкого к ближайшему. Так ближайшая по дате игра становится самым свежим pin'ом → Telegram показывает её в шапке pinned-сообщений сверху. Учитывает таймзону серии.
