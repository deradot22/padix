...
**Напоминание (`TelegramReminderService.tick`) — два канала:**
1. **ЛИЧНО** каждому участнику в его PRIVATE-чат, если `notify_reminder=true` на этом чате и `settings.enabled` + не quiet-hours.
2. **В ГРУППЫ автора события** (по `event.created_by_user_id`), у которых `notify_reminder=true`. Гейтится настройками **автора** (enabled, quiet-hours). Так один пинг в группе предупреждает всех вместо рассылки в личку. Чекбокс «напоминания за N часов» в `telegram-integration.tsx` теперь показывается и для групп (раньше только для приваток). По дефолту `notify_reminder=true` на entity — после деплоя группы автоматически начнут получать reminder; для отключения — выключить чекбокс в настройках.
