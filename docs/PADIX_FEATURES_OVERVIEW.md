...
**Порядок pin'ов:** строгий «ближайшая по дате — сверху» не делаем (это потребовало бы `unpin → pin`, который в некоторых Telegram-клиентах оставляет служебную строчку «Bot pinned a message» — нежелательно). Pin'ы остаются в порядке материализации (последний материализованный — сверху по шапке). На практике ближайшие игры обычно материализуются позже дальних (триггер ближе к старту), поэтому шапка чаще всего и так показывает ближайшую.
