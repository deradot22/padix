...
| `POST` | `/api/events/{id}/register` | Зарегистрировать игрока (по UUID). Гейт по рейтингу (min/max), см. 5.7 — самозапись не-автора вне диапазона → 409 |
...
`EventDetailsResponse` дополнительно содержит:
```
accessRestricted:  boolean    // true → PRIVATE без доступа, поля rounds/registeredPlayers/pendingCancelRequests пустые (см. 5.4)
```
...
### 5.4 Видимость (PRIVATE / PUBLIC)

`visibility` ∈ {`PRIVATE`, `PUBLIC`}. **По умолчанию `PUBLIC`** (открыто и для входа, и для просмотра деталей). Модель «видны всем, открыть могут не все»:

- **Листинги** (`/today`, `/upcoming`): отдаются **все эвенты** (включая PRIVATE) — чтобы юзер видел что игра существует и мог попросить пригласить.
- **`GET /api/events/{id}`** (детали):
  - PUBLIC → полный ответ.
  - PRIVATE для автора/зарегистрированного/приглашённого → полный ответ.
  - PRIVATE для **outsider** → облегчённый `EventDetailsResponse` с `accessRestricted=true`, базовая инфа (title, date, time, courts, authorName, registeredCount, visibility) есть, а `rounds`/`registeredPlayers`/`pendingCancelRequests` — пустые. Никаких имён участников, никаких счётов.
- **Логика доступа** — `EventService.filterVisibleFor(events, userId)` (используется и для листинга и для проверки одиночного эвента).

**Фронт**:
- В `/games` рядом с названием — иконка `🔒` (PRIVATE) / `🌐` (PUBLIC) (`V0GamesPage.VisibilityIcon`).
- При клике на PRIVATE-карточку outsider'ом → `V0EventPage` рендерит заглушку с замочком: «Приватная игра — попроси организатора пригласить».
...
| **Участник конкретного матча** (один из 4 игроков) | только `IN_PROGRESS` | Ввести счёт, **а также исправить свой же** счёт (`submitted_by_user_id == userId`), пока игра идёт |
| Участник эвента, но не этого матча | — | **403** |
| Не-автор | `FINISHED` | **403** |
| Не-автор, счёт введён **другим** участником | `IN_PROGRESS` | **409** `«Счёт уже введён другим участником. Изменить может только организатор.»` |

Участник может перезаписать **только собственную** запись (сравнение `existing.submittedByUserId == userId`); чужой счёт по-прежнему меняет лишь организатор.

`MatchResponse` отдаёт `submittedByMe: Boolean` (бэк считает `submittedByUserId == principalUserId()` в `EventController.getDetails`) — по нему фронт понимает, что зелёную карточку можно ещё исправить.

Фронт (`V0EventPage`): кнопка «Ввести счёт» при `IN_PROGRESS` показывается автору **и** любому участнику эвента. В модале раундов карточка матча, по которому уже введён счёт, окрашивается зелёным. `canSubmitScore = isAuthor || (isMyMatch && (!hasFinalScore || (submittedByMe && status===IN_PROGRESS)))` — свой финальный счёт остаётся кликабельным с подсказкой «Счёт введён вами — нажмите, чтобы исправить»; чужой финальный счёт `disabled` с подсказкой «Введён: <Имя>. Изменить может только организатор.». При клике на «Ввести счёт» не-автору авто-разворачивается раунд с его первым неввёденным матчем. На ошибке сохранения (включая 409 «гонка») вызывается `getEventDetails`, UI автоматически подтягивает актуальное состояние.

**Массовое редактирование счёта завершённой игры** — единый компонент [`web/src/components/edit-game-scores-dialog.tsx`](../web/src/components/edit-game-scores-dialog.tsx) (`EditGameScoresDialog`), используется и в `V0EventPage` (кнопка «Редактировать счёт» при `FINISHED`, видна только автору), и в `V0ProfilePage` (история игр). Раньше было две отдельные копии формы с разным дизайном — теперь одна (дизайн профиля: заголовок «Раунд N · Корт M», сортировка раундов/кортов, компоненты `Button`/`Input`). Диалог сам грузит `getEventDetails` и **централизованно ограничивает правку автором**: для не-автора форма открывается в режиме только чтения (заголовок «Счёт игры», инпуты `disabled`, кнопки «Сохранить» нет, баннер «Изменить счёт может только организатор»). Это совпадает с бэкендом (не-автор на `FINISHED` → 403).

**Полный пересчёт при правке счёта завершённого эвента** (`EventService.recomputeFinishedEvent`, зовётся из `submitScore` при `status==FINISHED` вместо старого поматчевого `recalculateMatchRatings`):
- **Идемпотентный пересчёт всего эвента** (не глобальный реплей): читает старые `rating_changes`, восстанавливает pre-event базу каждого игрока (oldRating его первого матча в порядке round→court), проигрывает все FINISHED-матчи заново по той же логике, что `finishEvent` (kFactor, `marginMultiplier`, `normByPlayer`, множитель калибровки), пересоздаёт `rating_changes`.
- **Текущий рейтинг корректируется на разницу** `player.rating += (newSum − oldSum)` — НЕ сбрасывается на pre-event, чтобы не стереть прогресс из последующих игр. `gamesPlayed` / `calibrationMatchesRemaining` повторно НЕ инкрементятся. Известные упрощения: kFactor и калибровочный множитель берутся от текущего состояния (идемпотентно, но не воспроизводит точное finish-time состояние).
- **`UserRatingNotification` пересоздаётся** с новым `delta`/`newRating` и `seenAt=null` → у всех участников всплывает плашка нового рейтинга (см. ниже live-доставку).
- **Telegram**: через `runAfterCommit` → `BotClient.notifyEventResultsUpdated` → `POST /api/internal/telegram/notify/event-results-updated`. Бот редактирует уже опубликованный RESULTS-пост (`editMessageText`), при невозможности — fallback delete+repost с обновлением `message_id`. Для этого добавлен дискриминатор `event_telegram_post.post_kind` (`ANNOUNCE`/`RESULTS`, **миграция V45**) — итоговый пост теперь сохраняется (раньше `recordForEventId=null`), а анонс-логика (пины/отмена/роутинг) фильтрует только `ANNOUNCE`.
- Тесты: `api/.../RatingRecomputeTest.kt` (инварианты: идемпотентность, `gamesPlayed` не растёт, коррекция = newSum−oldSum, бот вызван один раз) + `bot/.../TelegramServiceResultsTest.kt` (edit / fallback / repost). Оба зелёные.

**Live-доставка плашки нового рейтинга** (`web/src/ui/App.tsx`): помимо разового fetch на маунт добавлен polling `getRatingNotification` (~10с, пауза при `document.hidden`, немедленный запрос при возврате на вкладку). Поэтому после «Завершить игру» (или правки счёта) плашка `RatingNotificationModal` всплывает у всех участников с открытым приложением **без перезагрузки** в течение ~10с.
...
### 5.7 Ограничение по рейтингу при регистрации

Реализация: миграция V46 (`events.min_rating`, `events.max_rating`) + поля в `Event` / `CreateEventRequest` / `EventResponse` + проверка в `EventService.register`.

- Обе границы **опциональны и включительны**. Можно задать только `min`, только `max`, или обе. `null` — граница не задана. Валидация при создании: `min ≥ 0`, `max ≥ 0`, `min ≤ max`.
- Задаётся **при создании игры** (`V0CreateEventPage`): тогл «Ограничение по рейтингу» + поля Минимум/Максимум. Только для разовых игр (не для серий). Гейт считается по **Elo-рейтингу** (`Player.rating`).
- Гейт применяется **только к самозаписи не-автора**. Обходят ограничение (`bypassRatingGate` / автор):
  - организатор эвента (регистрирует любого через `register`, `byUserId == createdByUserId`);
  - `SocialService.addFriendToEvent` (автор добавляет друга) и `acceptInvite` (приглашённый организатором);
  - admin batch-регистрация.
  - Самозапись через Telegram-бота гейтится (сообщение об ошибке пробрасывается пользователю).
- При нарушении — `409 CONFLICT` с сообщением «Рейтинг X ниже минимального (…) / выше максимального (…)».
- Фронт (`V0EventPage`): бэйдж «Рейтинг {min–max}» в шапке; если рейтинг текущего юзера вне диапазона и он не автор — кнопка «Записаться» заменяется на неактивную «Рейтинг не подходит» с подсказкой попросить организатора.

---

### 5.8 Форматы игр (`EventFormat`)

Формат задаётся при создании игры (`V0CreateEventPage`, селектор «Формат игры», только для разовых игр — серии всегда AMERICANA). Enum `EventFormat`:

- **AMERICANA** (по умолчанию) — партнёры ротируются каждый раунд, анти-повтор (`PairingPlanner`). Все раунды создаются на старте (`planSchedule`). Есть под-режим `PairingMode` (ROUND_ROBIN / BALANCED).
- **MEXICANO** — пары каждый раунд формируются «змейкой» по текущей таблице очков (1+4 vs 2+3). **Инкрементальный**: `planSchedule` создаёт только раунд 1 (змейка по рейтингу), дальше организатор жмёт «+Раунд» (`addRound` → `planMexicanoNextRound`), пары считаются по `computeTournamentStandings`. Guard: следующий раунд нельзя создать, пока предыдущий не сыгран полностью (409). `roundsPlanned` — мягкая цель. Блок PairingMode скрыт, кнопка «Финальный раунд» скрыта (каждый раунд и так по таблице). Бэйдж «Мексикано» на странице игры.

- **FIXED_PAIRS** — партнёр фиксирован весь матч; пары играют **круговую** (round-robin, circle method — `FixedPairsPairing.round`). Всё расписание создаётся на старте (`planSchedule` ветка); `addRound` заблокирован. Регистрация — только парами: миграция **V47** (`registrations.team_id`), эндпойнт `POST /api/events/{id}/register-pair {player1Id, player2Id}` (`EventService.registerPair`, только организатор), одиночный `register` для FIXED_PAIRS запрещён (409). Берётся ≤ `courtsCount*2` пар. **Каскад отмены:** если один игрок пары отменяет регистрацию (`cancelRegistration`) или его убирает организатор (`removePlayer`) до старта, партнёр по общему `team_id` тоже переводится в CANCELLED — осиротевших REGISTERED не остаётся. **Safety net:** `startEvent` для FIXED_PAIRS проверяет, что все REGISTERED образуют полные пары (каждый `team_id` ровно дважды, нет REGISTERED без `team_id`), иначе 409 «В паре остался один игрок…» (защита от тихого дропа неполной пары в `fixedPairsTeams`). **Статус: бэкенд готов + протестирован; UI формы «Добавить пару» с пикером двух игроков — СДЕЛАН (`V0EventPage`, `api.registerPair`). Оставшийся гэп: в `EventDetailsResponse` по-прежнему нет `team_id`/списка пар, поэтому в ростере пары визуально НЕ сгруппированы (плоский список игроков).**

Реализация через ранние ветки по формату в `planSchedule`/`addRound` — путь AMERICANA не затрагивается. Snake-логика — чистый `SnakePairing.round`; round-robin пар — чистый `FixedPairsPairing.round` (оба покрыты юнит-тестами). Elo/`finishEvent` формат-агностичны (работают поматчево по teamA/teamB), правок не требуют.

Планируется (задача #8): **WINNER_COURT** (King of the Court — пары двигаются по кортам по результату).
...
**Закреп анонса (важно):** если у серии включён `pin_announcement` (или включён глобально в `TelegramUserSettings`), при материализации каждое созданное событие закрепляется в целевых чатах. **Каждая подписка/событие держит свой закреп независимо** — при материализации новой серии pin'ы других серий не снимаются (`TelegramService.unpinPreviousPinForEventInChat` снимает только pin того же `event_id` для случая ре-анонса). Раньше был баг: новый pin сносил все прежние Padix-pin'ы в чате (`unpinPreviousAnnouncementsInChat`) — исправлено.

**Порядок pin'ов в чате (по дате игры):** после каждого нового pin вызывается `reorderPinsByDate(chatId)`, которая для всех открытых будущих pin'ов в чате делает `unpin → pin` в порядке от далёкого к ближайшему. Ближайшая по дате игра становится самым свежим pin'ом → Telegram показывает её в шапке сверху. Учитывает таймзону серии.

**Бесшумные перепиновки (важно):** Telegram при каждом `pinChatMessage` вставляет в чат служебное сообщение «Bot pinned a message». Чтобы re-pin не спамил, в `handleUpdate` (polling) ловим такие служебные сообщения (`message.pinned_message != null` И `from.id == client.botId()` — т.е. закреп сделал наш же бот) и сразу делаем `deleteMessage`. Так и initial pin, и reorder, и любые повторные pin'ы от бота для людей в чате выглядят беззвучно (служебная строчка появляется на доли секунды и исчезает или вообще проходит мимо клиента). Реализация: `TgMessage.pinnedMessage` + `TelegramClient.deleteMessage` + `TelegramClient.botId()` (кэш после первого `getMe`).

**Backfill pin'ов для будущих игр** (одноразовая операция): `POST /api/telegram/admin/repin-upcoming` на bot-сервисе (защищён `X-Internal-Secret`, без `X-User-Id`). Пробегает по всем `event_telegram_post`, для записей с `pinned_message_id IS NULL` и `event.end_time` ещё впереди вызывает `pinChatMessage(disable_notification=true)` и сохраняет `pinned_message_id`. После — `reorderPinsByDate` в каждом затронутом чате. Идемпотентно (повторный вызов скипает уже-закреплённые). Возвращает `{pinned, skipped, reorderedChats}`. Полезно для миграции старых анонсов после фикса pin-логики, когда у них исторически не было pin или его сняла прежняя бажная логика.

**Автоснятие pin для прошедших игр:** cron `cleanupPastPins` (`@Scheduled(cron = "0 7 * * * *")`, раз в час) сканирует все посты с `pinned_message_id != null`, считает `endInstant` события в его таймзоне, и для тех, у кого игра уже закончилась — снимает pin и зануляет `pinned_message_id`. Так в чате остаются закреплёнными только актуальные и будущие игры.

**Целевые чаты подписки (`target_chat_ids`):** пустая строка = legacy-fallback «все группы автора» (бэк, `EventSeriesMaterializer`). На фронте при **редактировании** подписки `selectedTgChatIds` теперь = ровно сохранённый список (пусто → ничего не пред-выбрано). На **создании** новой подписки по-прежнему дефолт = все группы. Это убирает баг «при редактировании автоматом проставляется галка на все группы». Legacy-серии с пустым CSV: при первом редактировании пользователь явно отметит нужные группы → сохранится в БД, fallback больше не задействуется для них.
...
- **Политика неактивности** (`service/RatingDecay.kt`, cron `@Scheduled` в 03:00 UTC; дизайн 2026-07-03):
  - **>90 дней без матчей → повторная калибровка**: `calibrationMatchesRemaining` поднимается до 30 заново; на лидерборде «вопросик», правила как на калибровке (×1.5 к дельтам, вне фильтра «откалиброванные»);
  - **параллельно decay**: 1 очко в день вниз к target (медиана рейтинга игроков с ≥10 играми, fallback 1500), cap = 30% от `(baseline − target)`; **только вниз** — бездействие не может поднять рейтинг; **калибровка от decay НЕ защищает** (неактивность горит очками для всех); decay пишет запись `kind=DECAY` в rating_changes (V49) — на графике виден спад;
  - **>180 дней → рейтинг скрыт** из публичных мест: `ratingHidden` в `PlayerResponse`/`MeResponse` (лидерборд показывает «—?», скрытые сортируются в конец, профиль владельца — плашку «сыграй матч»); возвращается первым сыгранным матчем;
  - **идемпотентно**: decay считается от baseline — рейтинга на момент последнего МАТЧА (последняя матчевая запись rating_changes), ежедневные прогоны не компаундятся; хвостовая DECAY-запись переписывается;
  - разовый запуск вне крона: `-Dapp.maintenance.run-inactivity-job=true` (InactivityJobRunner).
...
Сравнители (порядок BALANCED исправлен 2026-07-03 — см. PAIRING_ALGORITHMS_AUDIT_2026-07-03.md):
- **ROUND_ROBIN** — `partnerRepeats → opponentRepeats → balanceViolations → totalBalance...`
- **BALANCED** — `partnerRepeats → balanceViolations → opponentRepeats → totalBalance...`
  (раньше `balanceViolations` стоял первым: при разнородных рейтингах единственный способ
  уложиться в cap=60 — пары «сильный+слабый», и алгоритм не разлучал их весь эвент;
  прод-кейс 17.05 — две пары вместе все 7 раундов).

Реализация: полный перебор разбиений с pruning на ≤3 кортах; жадный fallback на 4+ кортах.
**Seed детерминирован** от (eventId, состав, число существующих матчей) и логируется
(`[PAIRING] ... seed=`): превью и реальный план — один и тот же план, рассадку жалобы
можно воспроизвести.
...
- регрессия прод 17.05: cap=60, разнородные рейтинги — пары не залипают (max ≤ 2 из 7);
- один seed → идентичный план (превью == план).
...
Используется в модалке предупреждения «закрыть регистрацию». С 2026-07-03 превью
использует тот же детерминированный seed, что и planSchedule — обещанное число
раундов совпадает с фактическим (при неизменном составе).
...
- **rating-graph** — переключатель периодов (7д/30д/3м/всё), toggle режима оси X (по матчам / по времени), LTTB downsample при >60 точках, выбор сохраняется в localStorage. Hover/tap-тултип по точкам (рейтинг + дельта + дата). Ось Y зафиксирована слева; когда точек больше, чем помещается по ширине — плот расширяется и скроллится горизонтально (нативный пан, #11), скролл гасит тултип.
...
| 8 | Форматы игр (не турниры): **MEXICANO ✅ сделано** (§5.8); осталось FIXED_PAIRS (пары фиксированы, team_id) + WINNER_COURT. Групповой+плей-офф — отдельно как «турниры», позже | 🔥 в работе |
| ~~9~~ | ~~Ограничение по рейтингу при входе в эвент (min/max)~~ | ✅ сделано (V46 + гейт в `register`, см. §5.7) |
| ~~11~~ | ~~График: горизонтальный пан внутри периода~~ | ✅ сделано (нативный скролл + фикс. ось Y, `rating-graph.tsx`) |
